...
import { Menu, Dimmer } from 'semantic-ui-react';
import {
  StateHolder, Switch, Case, Default
} from '../utils';
...
const ContextMenu = ({
  ...
  <Dimmer
    ...
    active={show}
    onClick={onClose}
    style={{ position: 'fixed', background: '#00000010' }}
  >
    <Switch>
...
const DataGrid = ({
  columns, records, onChange, onDelete, onInsert, onDuplicate, onMatch
}) => {
  const dbColumns = columns.map(c => ({ readOnly: true, value: c.name, data: c }));
  const labels = [{ readOnly: true, value: '', width:'40px' }, ...dbColumns];
  const rows = records.map((record, i) => {
    const index = { readOnly: true, value: i + 1 };
    const data = columns.map(c => ({
      key: c.name,
      record,
      value: record.head[c.name],
      readOnly: record.deleted
    }));
    return [index, ...data];
  });
      ...
      {(state, invalidate) => (
        ...
        >
          ...
          <ContextMenu
            ...
            onMatch={
              state.selected && state.selected.canMatch
                ? () => onMatch(state.selected.column.data, state.selected.value)
                : null
            }
            onClose={() => invalidate({ showContextMenu: false })}
            onUndelete={
              state.selected && state.selected.record.deleted
                ? () => state.selected.record.deleted = false
                : null
            }
          />
        </div>
